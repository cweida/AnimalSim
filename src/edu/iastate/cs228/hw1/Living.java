...
public abstract class Living 
{
	...
	{		
		...
		for (int i = row-1; i < row + 1; i ++)
		{
			...
			{
				if (i < 0 || j < 0 || i > plain.getWidth()|| j > plain.getWidth())
				{
					continue;
				}
				
				Living k = plain.grid[i][j]; 
				
				if (k.who() == State.BADGER) 
				{
					population[BADGER] ++;
				}
				else if (k.who() == State.EMPTY) 
				{
					population[EMPTY] ++;
				}
				else if (k.who() == State.FOX) 
				{
					population[FOX] ++;
				}
				else if (k.who() == State.GRASS) 
				{
					population[GRASS] ++;
				}
				else
